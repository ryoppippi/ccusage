...
fn is_agent_command(command: &str) -> bool {
    matches!(
        command,
        "claude" | "codex" | "opencode" | "amp" | "droid" | "pi" | "copilot" | "gemini"
    )
}
...
fn droid_help() -> &'static str {
    "Usage: ccusage droid [OPTIONS] [daily|monthly|session]"
}
