...
use crate::{
    adapter::{amp, codex, copilot, droid, gemini, opencode, pi},
...
fn load_rows(kind: AgentReportKind, shared: &SharedArgs) -> Result<AllLoadResult> {
    ...
    if kind == AgentReportKind::Session {
        ...
        append_agent_rows(
            ...
            &mut detected_agents,
            "droid",
            load_droid_rows(AgentReportKind::Session, shared, &pricing)?,
        );
        append_agent_rows(
            &mut rows,
            &mut detected_agents,
    ...
    append_agent_rows(
        ...
        &mut detected_agents,
        "droid",
        load_droid_rows(AgentReportKind::Daily, shared, &pricing)?,
    );
    append_agent_rows(
        &mut rows,
        &mut detected_agents,
