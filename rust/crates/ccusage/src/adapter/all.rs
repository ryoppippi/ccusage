...
use crate::{
    adapter::{amp, codebuff, codex, copilot, gemini, opencode, pi},
...
fn load_rows(kind: AgentReportKind, shared: &SharedArgs) -> Result<AllLoadResult> {
    ...
    if kind == AgentReportKind::Session {
        ...
        append_agent_rows(
            ...
            &mut detected_agents,
            "codebuff",
            load_codebuff_rows(AgentReportKind::Session, shared, &pricing)?,
        );
        append_agent_rows(
            &mut rows,
            &mut detected_agents,
    ...
    append_agent_rows(
        ...
        &mut detected_agents,
        "codebuff",
        load_codebuff_rows(AgentReportKind::Daily, shared, &pricing)?,
    );
    append_agent_rows(
        &mut rows,
        &mut detected_agents,
