...
**ccusage** is a local CLI for understanding coding (agent) CLI token usage and estimated costs across Claude Code, Codex, OpenCode, Amp, pi-agent, Kilo, GitHub Copilot CLI, and Gemini CLI.

The original **“cc”** came from **C**laude **C**ode usage and now also fits **C**odex **C**LI usage. As OpenCode, Amp, pi-agent, Kilo, Gemini CLI, and other coding (agent) CLIs became part of the same workflow, ccusage expanded into a general name for local coding CLI usage analysis.
...
- **All Sources by Default** - Claude Code, Codex, OpenCode, Amp, pi-agent, Kilo, GitHub Copilot CLI, and Gemini CLI in one CLI
