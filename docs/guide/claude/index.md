...
ccusage can read Claude Code usage data as one of its supported local data sources. Claude Code is no longer treated as the only ccusage target; it uses the same unified and focused report model as Codex, OpenCode, Amp, pi-agent, GitHub Copilot CLI, and Kilo.
...
For Codex, OpenCode, Amp, pi-agent, GitHub Copilot CLI, and Kilo data locations, use the source-specific environment variables listed in [Environment Variables](/guide/environment-variables).
