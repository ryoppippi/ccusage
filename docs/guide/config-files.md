...
Use data source namespaces to set defaults and report overrides. Supported namespaces are `claude`, `codex`, `opencode`, `amp`, `pi`, `copilot`, `gemini`, and `openclaw`.
...
```json
{
	...
	"openclaw": {
		"defaults": {
			"openClawPath": "/path/to/.openclaw,/archive/.clawdbot"
		}
	}
}
```
