...
The flake uses `nix-filter` so only `.agents/skills` is included in the local skill source. Keep skill source files under `.agents/skills`; do not add global `agent-skills-nix` targets for this repository.
