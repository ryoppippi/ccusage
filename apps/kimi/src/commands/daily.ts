...
export const dailyCommand = define({
	name: 'daily',
	description: 'Show Kimi token usage grouped by day',
	args: sharedArgs,
	async run(ctx) {
		...
		if (events.length === 0) {
			log(jsonOutput ? JSON.stringify({ daily: [], totals: null }) : 'No Kimi usage data found.');
			return;
		}
		...
		logger.box(
			`Kimi Token Usage Report - Daily (Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
		);
