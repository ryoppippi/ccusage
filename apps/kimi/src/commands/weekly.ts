...
import { KimiPricingSource } from '../pricing.ts';
...
export const weeklyCommand = define({
	name: 'weekly',
	description: 'Show Kimi token usage grouped by week (ISO week format)',
	args: sharedArgs,
	async run(ctx) {
		...
		if (events.length === 0) {
			log(jsonOutput ? JSON.stringify({ weekly: [], totals: null }) : 'No Kimi usage data found.');
			return;
		}
		...
		logger.box(
			`Kimi Token Usage Report - Weekly (Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`,
		);
