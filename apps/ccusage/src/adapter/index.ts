...
import { detectOpenClaw, loadOpenClawRows } from './openclaw/index.ts';
...
export async function detectAllAgents(options: AdapterOptions): Promise<AgentId[]> {
	void options;
	const [claude, codex, opencode, amp, pi, openclaw] = resolveDetectedAgents(
...
async function loadAgentRowsWithoutProgress(
	...
	switch (agent) {
		...
		case 'openclaw':
			return loadOpenClawRows(kind, options);
	}
