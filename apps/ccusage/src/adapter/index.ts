...
import { detectGemini, loadGeminiRows } from './gemini/index.ts';
...
export async function detectAllAgents(options: AdapterOptions): Promise<AgentId[]> {
	void options;
	const [claude, codex, opencode, amp, pi, gemini] = resolveDetectedAgents(
...
async function loadAgentRowsWithoutProgress(
	...
	switch (agent) {
		...
		case 'gemini':
			return loadGeminiRows(kind, options, context);
	}
...
if (import.meta.vitest != null) {
	describe('agent adapter aggregation', () => {
		...
		it('defaults to every supported coding agent', () => {
			expect(resolveAllAgents({})).toEqual(['claude', 'codex', 'opencode', 'amp', 'pi', 'gemini']);
		});
	});
}
