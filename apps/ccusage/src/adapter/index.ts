...
import { detectDroid, loadDroidRows } from './droid/index.ts';
...
export async function detectAllAgents(options: AdapterOptions): Promise<AgentId[]> {
	void options;
	const [claude, codex, opencode, amp, pi, droid] = resolveDetectedAgents(
...
async function loadAgentRowsWithoutProgress(
	...
	switch (agent) {
		...
		case 'droid':
			return loadDroidRows(kind, options, context);
	}
...
if (import.meta.vitest != null) {
	describe('agent adapter aggregation', () => {
		...
		it('defaults to every supported coding agent', () => {
			expect(resolveAllAgents({})).toEqual(['claude', 'codex', 'opencode', 'amp', 'pi', 'droid']);
		});
	});
}
