...
def main [
    ...
    for required in ['head_dir', 'fixture_dir', 'codex_fixture_dir'] {
        if ($values | get $required) == null {
            error make {
                msg: $"--($required | str replace '_' '-') is required"
            }
        }
    }
    ...
    let base_dir = if $values.base_dir == null { null } else {
        $values.base_dir | path expand
    }
    ...
    let options = {
        ...
        head_runtime: $head_runtime
        head_runtime_description: (if $head_runtime == 'rust' and $head_native_bin_entry != null {
...
def parse_head_runtime [value: string] {
    if ($head_runtime_choices | any {|choice| $choice == $value }) {
        ...
        error make {
            msg: $"Invalid head runtime: ($value). Use package or rust."
        }
    }
}
def assert_sample_options [runs: int, warmup: int, label: string] {
    if $runs < 1 {
        error make {
            msg: $"--($label)runs must be a positive integer"
        }
    }
    if $warmup < 0 {
        error make {
            msg: $"--($label)warmup must be a non-negative integer"
        }
    }
}
def platform_name [] { match $nu.os-info.name {
    macos => 'darwin'
    windows => 'win32'
    $other => $other
} }
def arch_name [] { match $nu.os-info.arch {
    aarch64 => 'arm64'
    x86_64 => 'x64'
    $other => $other
} }
def package_bin_entry [repo_dir: string] {
    ...
    if $bin_path == null {
        error make {
            msg: $"ccusage bin is missing in ($package_dir)/package.json"
        }
    }
...
def installed_native_package_bin_entry [install_dir: string, target_platform?: string, target_arch?: string] {
    let platform = if $target_platform == null { platform_name } else { $target_platform }
    let arch = if $target_arch == null { arch_name } else { $target_arch }
...
def packed_tarball_size_bytes [repo_dir: string] {
    ...
    let result = (do {
        cd $package_dir
        run-external pnpm ...$pack_args | complete
    })
    $original_package_json | save --force $package_json_path
    if $result.exit_code != 0 {
        let message = (if ($result.stderr | str trim | is-not-empty) {
            $result.stderr | str trim
        } else {
            $result.stdout | str trim
        })
        error make {
            msg: $"pnpm pack failed: ($message)"
        }
    }
    let output_lines = ($result.stdout | lines)
    let json_start = (
        $output_lines | enumerate | where {|line| $line.item =~ '^\{' } | last | get index
    )
...
def remote_tarball_size_bytes [package_url: string] {
    ...
    if $result.exit_code != 0 {
        error make {
            msg: $"Failed to fetch ($package_url): ($result.stderr | str trim)"
        }
    }
...
def install_package_url [
    ...
    {
        private: true
        dependencies: {ccusage: $package_url}
    } | to json --indent 2 | save --force ([$install_dir, package.json] | path join)
    write_progress $"($label) package install started: ($package_url)"
    let started = date now
    let install_args = [install, --no-progress]
    let result = (do {
        cd $install_dir
        run-external bun ...$install_args | complete
    })
    ...
    if $result.exit_code != 0 {
        ...
        error make {
            msg: $"($label) package install failed: ($message)"
        }
    }
...
def write_progress [message: string] { print --stderr $"[ccusage-perf] ($message)" }
def git_sha [directory: string] {
    (do {
        cd $directory
        run-external git rev-parse HEAD | complete
    }).stdout | str trim
}
def format_sha [sha: string] {
    if ($sha | str length) > 12 {
        $sha | str substring 0..<12
    } else { $sha }
}
...
def format_size [bytes: number] { $"($bytes / 1024 | into string --decimals 2) KiB" }
...
def create_ccusage_command_from_rust_binary [
    bin_entry: string
    fixture_dir: string
    codex_fixture_dir
    command: string
...
def measure_command_milliseconds [args: list<string>, command_env: record, label: string] {
    ...
    if $result.exit_code != 0 {
        error make {
            msg: $"($label) failed: ($result.stderr)"
        }
    }
    $elapsed
}
...
def time_command_args [command] {
    if (platform_name) == 'linux' {
        {
            cmd: '/usr/bin/time'
            args: ([-v] | append $command.args)
        }
    } else if (platform_name) == 'darwin' {
        {
            cmd: '/usr/bin/time'
            args: ([-l] | append $command.args)
        }
    } else {
        null
    }
...
def measure_command_peak_rss_bytes [command, label: string] {
    ...
    if $timed == null {
        error make {
            msg: $"Peak RSS measurement is not supported on (platform_name)"
        }
    }
...
def compare_command [command: string, options] {
    ...
    if $result.exit_code != 0 {
        error make {
            msg: $"hyperfine failed for ($options.fixture_title) / ($command): exit ($result.exit_code)\n($result.stderr)"
        }
    }
    ...
    let base_memory = (measure_command_memory $base_command {
        label: $"($options.fixture_title) / ($command) base"
        runs: $options.memory_runs
    })
    let head_memory = (measure_command_memory $head_command {
        label: $"($options.fixture_title) / ($command) PR"
        runs: $options.memory_runs
    })
...
def compare_fixture [options] {
    ...
    mut results = []
    for command in $options.commands { $results = ($results | append (compare_command $command ($options | upsert fixture_title $options.title))) }
...
def compare_runtime_diagnostic_command [command: string, options] {
    ...
    if $result.exit_code != 0 {
        error make {
            msg: $"hyperfine failed for ($options.fixture_title) / runtime diagnostics ($command): exit ($result.exit_code)\n($result.stderr)"
        }
    }
    let hyperfine_output = (open $export_path)
    if ($hyperfine_output.results | length) != ($options.variants | length) {
        error make {
            msg: $"hyperfine reported ($hyperfine_output.results | length) runtime diagnostic results for ($command), expected ($options.variants | length)"
        }
    }
...
def format_fixture_path [head_dir: string, fixture_dir: string] {
    let relative = (try {
        $fixture_dir | path relative-to $head_dir
    } catch { $fixture_dir })
    if ($relative | str starts-with '..') { $fixture_dir } else { $relative }
}
def format_fixture_stats [stats] { $"(format_data_size $stats.bytes), ($stats.files | into string) files" }
...
def render_markdown [sections, sizes, options] {
    ...
    $lines = ($lines | append ['## ccusage performance comparison', ''])
    if $options.head_sha? != null {
