...
export function toDateKey(timestampMs: number, timezone?: string): string {
...
export function toMonthKey(timestampMs: number, timezone?: string): string {
	const tz = safeTimeZone(timezone);
	const date = new Date(timestampMs);
	const formatter = new Intl.DateTimeFormat('en-CA', {
		year: 'numeric',
...
export function formatDisplayDateTime(timestampMs: number, locale?: string, timezone?: string): string {
	const tz = safeTimeZone(timezone);
	const date = new Date(timestampMs);
	const formatter = new Intl.DateTimeFormat(locale ?? 'en-US', {
		dateStyle: 'short',
