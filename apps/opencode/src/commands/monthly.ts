...
export const monthlyCommand = define({
	...
	async run(ctx) {
		...
		if (events.length === 0) {
			log(jsonOutput ? JSON.stringify({ monthly: [], totals: null }) : 'No OpenCode usage data found.');
			return;
		}
		...
		try {
			...
			if (rows.length === 0) {
				log(jsonOutput ? JSON.stringify({ monthly: [], totals: null }) : 'No OpenCode usage data found for provided filters.');
				return;
			}
			...
			logger.box(`OpenCode Token Usage Report - Monthly (Timezone: ${ctx.values.timezone ?? DEFAULT_TIMEZONE})`);

			const table: ResponsiveTable = new ResponsiveTable({
				head: ['Month', 'Models', 'Input', 'Output', 'Reasoning', 'Cache Read', 'Cache Write', 'Total Tokens', 'Cost (USD)'],
