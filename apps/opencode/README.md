...
> Analyze [OpenCode](https://github.com/opencode-ai/opencode) usage logs with the same reporting experience as `ccusage`.

> ⚠️ **Compatibility wrapper:** Prefer `bunx ccusage@latest opencode`. This package now prints a deprecation warning and forwards to `ccusage opencode`.
...
bunx ccusage@latest opencode daily

# Weekly usage grouped by ISO week
bunx ccusage@latest opencode weekly

# Monthly usage grouped by month
bunx ccusage@latest opencode monthly

# Session-level detailed report
bunx ccusage@latest opencode session

# JSON output for scripting
bunx ccusage@latest opencode daily --json

# Compact mode for screenshots/sharing
bunx ccusage@latest opencode daily --compact
```
